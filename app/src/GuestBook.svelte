<script>
    ...
    async function handleSubmit() {
        ...
        /*after get image id add to main sheet*/
        await axios.post('/api/appendDataToTempSheet', {
            date: d.getDate()+"/"+d.getMonth()+"/"+d.getFullYear(),
            time: d.getHours()+":"+d.getMinutes()+":"+d.getSeconds(),
            name: name,
            gender: gender,
            ...
            .then(function (response){
            if (response.status === 200){
                isWriteToSecondarySheetComplete = true;
                console.log("complete to create data at main sheet")
            }
        })

        if (isUploadImageComplete && isWriteToMainSheetComplete && isWriteToSecondarySheetComplete) {
...
<div class="container-2xl py-10 px-14 grid flex items-center justify-items-center h-screen m-0">
    <div class="w-full">
        <h1 class="text-4xl text-center font-bold">Guest Book</h1>
        <form on:submit|preventDefault={handleSubmit}>
            <div class="w-full shadow-2xl bg-white rounded mt-8 px-5 py-5">
                ...
                <div class="w-full grid justify-items-center grid-cols-1 mt-3">
                    ...
                    {:else }
                        <button class="w-full bg-zinc-800 hover:bg-zinc-600 rounded-md text-white px-1.5 py-1.5" type="submit">Submit</button>
                    {/if}
                </div>
            </div>
        </form>

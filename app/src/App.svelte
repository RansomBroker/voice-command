...
{:else}
	<div class="container-2xl py-10 px-5 grid flex items-center justify-items-center h-screen m-0 animate-fade">
		...
		{/if}
		<!--Mic Button -->
		<button bind:this={camera}  on:click={handleStartClick}  class="rounded-full {isClicked ? 'bg-[#60F5FF]':'shadow-lg md:shadow-xl xl:shadow-xl  bg-white'} {isAudioCancel ? 'bg-[#FF6060]' : ''} w-28 h-28 sm:w-32 sm:h-32 md:h-32 md:w-32 xl:h-36 xl:w-36 z-50 relative z-10 p-0 m-0" disabled={isClicked} >
